...
URL = "https://avtobazar.ua/poisk/avto/?country1=1911&show_only=only_used&per-page=10"
EXCHANGE_RATES = 1.12
...
class SearchPageElements(unittest.TestCase):
	...
	#1 Клик на Поиск верхнего блока
	...
	#1 Сбор данных в списке объявлений в словарь
	...
	#1 Собираем линки на конечные страницы объявлений
	def output_link_ads_list(self):
		driver = self.driver
		ad_div = driver.find_elements(By.CSS_SELECTOR, "div[class='row advert-item'] * div.h4 a")
		link_list = [i.get_attribute('href') for i in ad_div]
		return link_list

	#1 Установка диапазона велечин
	...
	#1 Выбор из выпадающего спсика поиск по шаблону
	...
	#1  Привести цены евро в доллары
	...
	#1 Хелпер, для обработки совпадений для regex
